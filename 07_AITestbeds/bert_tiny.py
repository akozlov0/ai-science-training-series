...
def evaluate_bert_tiny(rebuild_policy=None, should_execute=True):
    ...
    # Custom inputs
    inputs = {
        "input_ids": torch.randint(1000, (batch_size, max_seq_length), dtype=torch.long),
        "attention_mask": torch.randint(1, (batch_size, max_seq_length), dtype=torch.bool),
    }

    # generate groq model
    groq_model = groqit(pytorch_model, inputs, rebuild=rebuild_policy)
